...
if __name__ == "__main__":
    ...
    # Cria processador de contexto
    processador = ProcessadorContexto()
    processador.adicionar_contexto_global("ambiente", "producao")
    processador.adicionar_contexto_global("versao", "1.2.3")
    
    # Cria analisador de fluxo
    analisador = AnalisadorFluxoContínuo(tamanho_janela=50)
    
    # Registra callbacks
    def processar_metrica(metrica):
        # Processa contexto
        metrica_processada = processador.processar(metrica)
        # Adiciona ao analisador de fluxo
        analisador.processar_metrica(metrica_processada)
        # Imprime métrica
        print(f"Métrica processada: {metrica_processada.nome} = {metrica_processada.valor} {metrica_processada.unidade}")
    
    coletor_throughput.registrar_callback(processar_metrica)
    coletor_erros.registrar_callback(processar_metrica)
    coletor_latencia.registrar_callback(processar_metrica)
    ...
    try:
        for i in range(30):
            ...
            if random.random() < 0.3:
                categoria = random.choice(["http", "database", "auth"])
                coletor_erros.registrar_erro(categoria=categoria, contexto={"codigo": random.randint(400, 500)})
            
            # Simula latência
            latencia = random.uniform(10, 500)
            coletor_latencia.registrar_latencia(latencia, contexto={"endpoint": "/api/data"})
        ...
        # Para coletores
        coletor_throughput.parar()
        coletor_erros.parar()
        coletor_latencia.parar()
        
        # Imprime estatísticas finais
        print("\nEstatísticas finais:")
        estatisticas = analisador.calcular_estatisticas_janela("throughput")
        print(f"Throughput: {estatisticas}")
        
        estatisticas = analisador.calcular_estatisticas_janela("erros")
        print(f"Erros: {estatisticas}")
        
        estatisticas = analisador.calcular_estatisticas_janela("latencia")
        print(f"Latência: {estatisticas}")
